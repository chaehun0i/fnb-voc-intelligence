...
from src.ingestion.file_adapters import (
    ingest_reviews,
    map_product,
    map_review,
    read_file,
...
def test_review_ingestion_reports_invalid_rows(tmp_path: Path) -> None:
    path = tmp_path / "reviews.jsonl"
    path.write_text('{"id":"R1","product":"P1","rating":8,"text":"좋아요","date":"2026-01-01"}\n', encoding="utf-8")
    reviews, errors = ingest_reviews(path, "jsonl", "partner", REVIEW_MAP)
    assert reviews == [] and errors[0].startswith("row 2:")
